...
Afin de déterminer l'arbre binaire de recherche optimal, nous sélectionnons pas à pas les noeuds de probabilité d'apparition maximale. Pour cela, pour une entrée à $n$ valeurs nous commencons par stocker chaque sommet sur la diagonale d'une matrice $(n*n)$. Ensuite dans les cases de la matrice de la diagonale supérieure nous sélectionnons le nouveau sommet entre les deux cases adjacentes déjà remplies qui aura la probabilité d'apparition la plus importante dans l'arbre composé de ces 2 sommets. On passe ensuite à la diagonale supérieure et on effectue la même procédure mais pour les arbres à 3 éléments, etc... Simultanément, nous stockons le nombre moyen de comparaisons ($p_i * profondeur$) effectué dans chaque arbre dans une seconde matrice.

Une fois la matrice construite, on déduit l'arbre de recherche optimal pas à pas en reparcourant les 2 matrices dans le sens inverse de remplissage.

Cette méthode est basée sur l'algorithme de Knuth et s'effectue de manière itérative.
